...
async def decidepo(embed, text, commandInfo):
    ...
    if not 'PO' in serverSettings:
        ...
        return embed
    todelete = []
    transChannel = shared_info.bot.get_channel(int(str(serversList[str(commandInfo['serverId'])]['fachannel']).replace('<#', '').replace('>', '')))
        ...
        if offerPlayer['draft']['round'] == 1 and isresigned and offerPlayer['draft']['year'] + export['gameAttributes']['rookieContractLengths'][0] == export['gameAttributes']['season']:
            ...
            if legalOffer:
                ...
                if serversList[str(commandInfo['serverId'])]['openmarket'] == 'on':
                    ...
                    channelId =int(str(serversList[str(commandInfo['serverId'])]['fachannel']).replace('<#', '').replace('>', ''))
...
async def bulkoffermins(embed, text, commandInfo):
    if commandInfo['userTid'] < 0:
        embed.add_field(name="No team", value="You need a team first.")
        return embed

    count = 10
    sort_by = 'ovr'
    if len(text) > 1 and text[1].lower() == 'pot':
        sort_by = 'pot'
    ...
    currentOfferCount = sum(1 for o in offerList if o['team'] == commandInfo['userTid'])
    if currentOfferCount >= 70:
        embed.add_field(name="Too Many Offers", value=f"You already have {currentOfferCount} offers. You must have fewer than 70 offers to use this command.")
        return embed
    minContract = export['gameAttributes']['minContract'] / 1000
    holdoutPct = float(serversList[commandInfo['serverId']]['holdout']) / 100

    existingPids = set(o['player'] for o in offerList if o['team'] == commandInfo['userTid'])

    eligible = []
    for p in players:
        if p['tid'] == -1:
            askingPrice = p['contract']['amount'] / 1000
            holdoutLimit = holdoutPct * askingPrice
            if holdoutLimit <= minContract and p['pid'] not in existingPids:
                eligible.append(p)

    eligible.sort(key=lambda p: p['ratings'][-1][sort_by], reverse=True)
    eligible = eligible[:count]

    priority = sum(1 for o in offerList if o['team'] == commandInfo['userTid']) + 1

    created = []
    createdPlayers = []
    for p in eligible:
        offer = {
            'player': p['pid'], 'amount': minContract,
            'years': 1, 'team': commandInfo['userTid'],
            'option': None, 'priority': priority, 'qo': False
        }
        offer['score'] = round(await free_agency_runner.offer_score(
            offer, commandInfo['serverId']), 3)
        offerList.append(offer)
        name = p['firstName'] + ' ' + p['lastName']
        stat = p['ratings'][-1][sort_by]
        label = 'OVR' if sort_by == 'ovr' else 'POT'
        created.append(f"{name} ({stat} {label})")
        createdPlayers.append({'pid': p['pid'], 'name': name})
        priority += 1
    ...
    if created:
        ...
        if len(resultText) > 1024:
            resultText = resultText[:1020] + '...'
        embed.add_field(
            name=f'Bulk Min Offers ({len(created)})',
            value=resultText + f'\n\nAll at ${minContract}M/1Y', inline=False
        )

        # Post to FA channel
        serverSettings = serversList[str(commandInfo['serverId'])]
        if serverSettings['openmarket'] == 'on' or serverSettings['semiopenmarket'] == 'on':
            try:
                channelId = int(str(serverSettings['fachannel']).replace('<#', '').replace('>', ''))
                channel = shared_info.bot.get_channel(channelId)
                if isinstance(channel, discord.TextChannel):
                    if serverSettings['openmarket'] == 'on':
                        # Open FA: post Top 5 Offers per player (same as regular offer)
                        for o in offerList:
                            o['score'] = await free_agency_runner.offer_score(o, commandInfo['serverId'])
                        for cp in createdPlayers:
                            playerOffers = []
                            for o in offerList:
                                if int(o['player']) == int(cp['pid']):
                                    optionText = f", + {o['option']}" if o.get('option') else ""
                                    playerOffers.append({
                                        "team": o['team'],
                                        "score": o['score'],
                                        "offer": f"${o['amount']}M/{o['years']}Y{optionText}"
                            ...
                            playerOffers = sorted(playerOffers, key=lambda o: o['score'], reverse=True)
                            text_msg = f">>> **{cp['name']} - Top 5 Offers**\n"
                            for o in playerOffers[:5]:
                                ...
                                for t in export['teams']:
                                    if t['tid'] == o['team']:
                                        teamName = t['region'] + ' ' + t['name']
                                        abbrev = t['abbrev']
                                roleMention = basics.team_mention(commandInfo['message'], teamName, abbrev)
                                text_msg += f"• {roleMention} - {o['offer']} (score: **{round(o['score'], 2)}**)\n"
                            await channel.send(text_msg)
                    else:
                        # Semi-open FA: just post offer counts
                        lines = []
                        for cp in createdPlayers:
                            offerCount = sum(1 for o in offerList if int(o['player']) == int(cp['pid']))
                            offerword = "offer" if offerCount == 1 else "offers"
                            lines.append(f"**{cp['name']}** now has {offerCount} {offerword}.")
                        header = "Alert: New bulk min offers submitted.\n"
                        msg = header
                        for line in lines:
        ...
        embed.add_field(
            ...
            value='No free agents found who would accept a minimum offer (or you already have offers for all of them).'
        )
    return embed
    ...
    for u in userOffers:
        ...
        for p in players:
            if p['pid'] == u['player']:
                ...
                ovr = p['ratings'][-1]['ovr']
                pot = p['ratings'][-1]['pot']
                playerRatings = f" ({ovr}/{pot})"
                skills = p['ratings'][-1].get('skills') or []
                if skills:
                    badgeText = f" *{' '.join(skills)}*"
        if u['option'] != None:
            optionText = f', + {u["option"]}'
        else:
            optionText = ""
        text = f"Pri {u['priority']} - {playerName}{playerRatings}{badgeText}, ${u['amount']}M/{u['years']}Y{optionText}"
...
async def runfa(embed, text, commandInfo):
    ...
    def check(payload):
        return str(payload.emoji) == '✅' and payload.user_id == commandInfo['message'].author.id and payload.message_id == confirmEmbed.id
    ...
    if export['gameAttributes']['phase'] == 7:
        for team in export['teams']:
            if team['disabled'] == False and not team['tid'] in occupiedteams:
                ...
                #print(int(str(serversList[str(commandInfo['serverId'])]['fachannel']).replace('<#', '').replace('>', '')))
                transChannel = shared_info.bot.get_channel(int(str(serversList[str(commandInfo['serverId'])]['fachannel']).replace('<#', '').replace('>', '')))
...
async def autosign(embed, text, commandInfo):
    ...
    offerList = serversList[commandInfo['serverId']]['offers']
    if len(offerList) > 0:
        embed.add_field(name='Offer list not empty', value='Clear offers before running -autosign.', inline=False)
        return embed
    for p in export['players']:
        if p['tid'] == -1:
...
async def autofa(embed, text, commandInfo):
    """Auto-create offers for teams with no existing offers to reach min roster."""
    export = shared_info.serverExports[commandInfo['serverId']]
    players = export['players']
    teams = export['teams']
    offerList = serversList[commandInfo['serverId']]['offers']
    serverSettings = serversList[str(commandInfo['serverId'])]
    minRosterSize = export['gameAttributes']['minRosterSize']
    minContract = export['gameAttributes']['minContract']
    holdoutMultiplier = float(serverSettings.get('holdout', 100)) / 100

    # Find which teams already have offers
    teamsWithOffers = set(o['team'] for o in offerList)

    # Get min-contract free agents sorted by OVR
    fa = [p for p in players if p['tid'] == -1
          and p['contract']['amount'] == minContract]
    fa.sort(key=lambda x: -x['ratings'][-1]['ovr'])
    faIndex = 0

    # Create offers for teams with no offers that are under min roster
    teamNames = []
    totalOffers = 0
    for t in teams:
        if not t['disabled'] and t['tid'] not in teamsWithOffers:
            rostered = sum(1 for p in players if p['tid'] == t['tid'])
            needed = minRosterSize - rostered
            if needed > 0:
                # Set toSign so run_fa2 caps signings at what's actually needed
                serverSettings['toSign'].update({t['tid']: needed})
                # Create 2x offers for backup options
                offersToMake = needed * 2
                pri = 1
                for i in range(offersToMake):
                    if faIndex >= len(fa):
                        break
                    p = fa[faIndex]
                    faIndex += 1
                    offerAmount = max(minContract, holdoutMultiplier * p['contract']['amount']) / 1000
                    offer = {
                        'player': p['pid'], 'amount': offerAmount,
                        'years': 1, 'team': t['tid'],
                        'option': None, 'priority': pri, 'qo': False
                    }
                    offer['score'] = round(await free_agency_runner.offer_score(
                        offer, commandInfo['serverId']), 3)
                    offerList.append(offer)
                    pri += 1
                    totalOffers += 1
                teamNames.append(f"{t['region']} {t['name']} ({needed} spots, {min(offersToMake, totalOffers)} offers)")

    if not teamNames:
        embed.add_field(name='No Teams Need Auto-Offers', value='All teams either have offers or are at minimum roster size.')
    else:
        await basics.save_db(serversList)
        embed.add_field(name=f'Auto-Offers Created ({totalOffers} total)', value='\n'.join(teamNames) + '\n\nRun **-runfa** when ready.')
